...
Smits.PhyloCanvas.NewickParse.prototype = {
	...
	recursiveParse = function(clade, parentNode){
		...
		if(clade.name){
			...
			// MODIFICATION:  allow bgStyle to be specified as an element in the clade if it cannot be made an attribute on the clade's name element
			if (clade.bgStyle) {
				node.bgStyle = clade.bgStyle[0].Text;
			}	
